...
export const GET = route("GET /api/items", async () => {
  ...
  const catalog = items.flatMap((item) => {
    ...
    return item.purchaseBatches.map((batch) => ({
      ...
      // Warranty needs both gates open: the product must be eligible, and this
      // particular batch must have come with supplier cover. A batch bought
      // without cover cannot issue a warranty even for an eligible product.
      warrantyEligible: item.warrantyEligible,
      requiresSerial: item.warrantyEligible && item.requiresSerial,
      warrantyMonths: item.warrantyEligible ? batch.warrantyMonths : null,
    }));
  });
...
export const POST = route("POST /api/items", async (req) => {
  ...
  const item = await prisma.item.create({
    data: {
      ...
      stockQty: 0,
      warrantyEligible: body.warrantyEligible,
      defaultWarrantyMonths: body.warrantyEligible
        ? (body.defaultWarrantyMonths ?? null)
        : null,
...
export const PUT = route("PUT /api/items", async (req) => {
  ...
  const item = await prisma.item.update({
    ...
    data: {
      ...
      reorderLevel: body.reorderLevel,
      sellingPrice: body.sellingPrice,
      ...(body.warrantyEligible !== undefined
        ? {
            warrantyEligible: body.warrantyEligible,
            // Turning eligibility off clears the default term too, so a later
            // re-enable does not silently resurrect an old number.
            defaultWarrantyMonths: body.warrantyEligible
              ? (body.defaultWarrantyMonths ?? null)
              : null,
          }
        : {}),
      ...(body.requiresSerial !== undefined
        ? { requiresSerial: body.requiresSerial }
        : {}),
    },
  });
